...
Hours spent: 2
Any difficulties?: Trying to understand backtracing to see output :)
Complexity of this algorithm: O(n^(n^2))
***************************************************/
#include <iostream>
	...
	if(s[r][c] == 0)//If the current slot is 0, look for a number that works
	  {
	    for(int num = 1; num <= SIZE; num++) //a number can be 1 through SIZE
	      {
		 if(valid(s, r, c, num) != false) //call valid to see if the current number is possible
		  {
