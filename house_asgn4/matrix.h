...
Hours spent?: 3
Any difficulties?: Went to ELB for extra help by CS 311 tutor
***********************************/
#include <iostream>
#include <cstdlib>
#include <fstream>
...
{
  // creates the rows and columns of matricies
...
{
   // allocates spaces for each row and column
   for (int i = 0; i < mrx.R; i++) {
     for (int j = 0; j < mrx.C; j++) {
       // use setw(10) to allocate 10 spaces to show each value
       o << setw(10) << mrx.m[i][j];
     }
     // ends the line and goes to next space
     o << endl;
   }
   return o;
...
Matrix<T> Matrix<T>::operator*(const Matrix<T>& other)
{
  ...
  // cannot compute because of incomparable sizes
  // throw an exception
  if(this -> C != other.R) {
   throw size_error();
  }
  ...
  // Otherwise compute
  Matrix new_matrix(this -> R, other.C);

  // Nested foor loop that iterates through the matricies
  for(int i = 0; i < this -> R;i++) {
    for(int j = 0; j < other.C; j++) {
      for (int k = 0; k < this -> C; k++) {
        // Multiplies the matricies
        new_matrix.m[i][j] += this -> m[i][k] * other.m[k][j];
      }
    }
  }
  return new_matrix;
...
Matrix<T>::~Matrix()
{
  // destroy what is created in heap memory                                       // if m is not NULL do the following
  if (m != NULL) {
    for (int i = 0; i < R; i++) {
      // delete instances of m
      delete[]m[i];
    }
    delete[]m;
  }                                                                             
